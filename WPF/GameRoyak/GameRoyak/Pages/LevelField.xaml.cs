...
namespace GameRoyak.Pages
{
    public partial class LevelField
    {
        ...
        private void InitialStartValues()
        {
            ...
            SoundSteps.Open(new Uri(FilePath.Combine(Settings.DirectoryJsons, "Audio/Steps.mp3")));
            SoundBackground.Open(new Uri(FilePath.Combine(Settings.DirectoryJsons, "Audio/Background.mp3")));
            ...
            for (var i = 0; i < FieldProvider.NumColumn; i++)
            {
                for (var j = 0; j < FieldProvider.NumRow; j++)
                {
                    ...
                    else
                    {
                        var stateCell = new Image
                        {
                            Source = new BitmapImage(new Uri(FilePath.Combine(Settings.MainDirectory, "Images/start.png"))),
                            Stretch = Stretch.Fill
                        };
        ...
        private void PaintCurrentAndPreviousCell()
        {
            ...
            currentCell.Source = new BitmapImage(
                new Uri(FilePath.Combine(Settings.MainDirectory,
