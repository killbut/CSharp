...
namespace GameRoyak.Pages
{
    ...
    {
        private static readonly string _directory = Directory.GetParent(Directory.GetParent(Environment.CurrentDirectory).FullName).Parent.FullName + "/GameRoyak/Images/";
        ...
        public MainMenu(Player player)
        {
            ...
            MenuGrid.Background = new ImageBrush(new BitmapImage(new Uri(FilePath.Combine(_directory, $"BackgroundMenu.jpg"))));
            if (StatePage.IsStartGame)
            {
                StartButton.IsEnabled = false;
                StartButtonText.Text = "Старт";
                var classes = JObject.Parse(File.ReadAllText(FilePath.Combine(Settings.DirectoryJsons, "PlayerClasses.json")));
        ...
        private void ButtonClass_Click(Button senderButton, Grid allButtons)
        {
            ...
            foreach (var item in classes)
            {
                if (Player.Class == item.Key)
                {
                    ...
                    Player.Icon =
                        new BitmapImage(new Uri(FilePath.Combine(Settings.MainDirectory, item.Value["icon"].ToString())));

                    var image = new BitmapImage();
                    image.BeginInit();
                    image.UriSource = new Uri(FilePath.Combine(Settings.MainDirectory, item.Value["image"].ToString()));
                    image.EndInit();
